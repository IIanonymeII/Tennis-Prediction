...
parser.add_argument('--year', help='yeau to study (int)', default=2018) #choisir le nom du ficher
args = parser.parse_args()
year_to_study = int(args.year)

with open('Data_year/Data_%i' % year_to_study, 'rb') as file:
...
with open('indicators_dicts_year/indicators_dicts_%i' % year_to_study, 'wb') as file:
    my_pickler = pickle.Pickler(file)
    my_pickler.dump([tournaments_already_visited, level_dict, {'Hard': 4, 'Grass': 3, 'Clay': 1, 'Carpet': 2}, extrema_dict])

with open('reversed_indicators_dicts_year/reversed_indicators_dicts_%i' % year_to_study, 'wb') as file:
    my_pickler = pickle.Pickler(file)
